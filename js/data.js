...
const smokeStages = [ { ms: 0, label: "0분", msg: "몸이 회복을 시작합니다." }, { ms: 20 * MINUTE, label: "20분", msg: "혈압과 맥박이 안정됩니다." }, { ms: 30 * MINUTE, label: "30분", msg: "대화 시 느껴지던 입 냄새가 줄어듭니다." }, { ms: 1 * HOUR, label: "1시간", msg: "금단 증상이 시작될 수 있습니다." }, { ms: 6 * HOUR, label: "6시간", msg: "심장이 조금 편해집니다." }, { ms: 12 * HOUR, label: "12시간", msg: "산소 수치가 회복됩니다." }, { ms: 24 * HOUR, label: "24시간", msg: "몸과 옷에 밴 냄새가 옅어집니다." }, { ms: 2 * DAY, label: "2일", msg: "주변 사람들이 느끼는 담배 냄새가 줄어듭니다." }, { ms: 3 * DAY, label: "3일", msg: "니코틴이 대부분 배출됩니다." }, { ms: 4 * DAY, label: "4일", msg: "폐가 정화를 시작합니다." }, { ms: 5 * DAY, label: "5일", msg: "금단 증상이 줄어들 수 있습니다." }, { ms: 7 * DAY, label: "7일", msg: "일주일을 버텼습니다." }, { ms: 10 * DAY, label: "10일", msg: "호흡이 조금 편해집니다." }, { ms: 14 * DAY, label: "14일", msg: "폐 기능이 개선됩니다." }, { ms: 21 * DAY, label: "21일", msg: "의존이 약해지기 시작합니다." }, { ms: 30 * DAY, label: "30일", msg: "재흡연 위험이 감소합니다." }, { ms: 60 * DAY, label: "60일", msg: "몸의 변화가 느껴집니다." }, { ms: 90 * DAY, label: "90일", msg: "체력이 좋아질 수 있습니다." }, { ms: 180 * DAY, label: "180일", msg: "면역력이 회복됩니다." }, { ms: 365 * DAY, label: "365일", msg: "심장질환 위험이 크게 감소합니다." }, { ms: 730 * DAY, label: "730일", msg: "비흡연자 수준에 가까워집니다." } ];
const drinkStages = [ { ms: 0, label: "0시간", msg: "간이 회복을 시작합니다." }, { ms: 6 * HOUR, label: "6시간", msg: "몸이 안정되기 시작합니다." }, { ms: 12 * HOUR, label: "12시간", msg: "탈수 상태가 개선됩니다." }, { ms: 24 * HOUR, label: "24시간", msg: "알코올이 대부분 분해됩니다." }, { ms: 2 * DAY, label: "2일", msg: "수면이 좋아질 수 있습니다." }, { ms: 3 * DAY, label: "3일", msg: "집중력이 회복됩니다." }, { ms: 5 * DAY, label: "5일", msg: "피부가 맑아질 수 있습니다." }, { ms: 7 * DAY, label: "7일", msg: "간이 회복되기 시작합니다." }, { ms: 10 * DAY, label: "10일", msg: "염증 수치가 감소합니다." }, { ms: 14 * DAY, label: "14일", msg: "소화 기능이 개선됩니다." }, { ms: 21 * DAY, label: "21일", msg: "습관이 약해지기 시작합니다." }, { ms: 30 * DAY, label: "30일", msg: "간 기능이 개선됩니다." }, { ms: 45 * DAY, label: "45일", msg: "에너지가 안정됩니다." }, { ms: 60 * DAY, label: "60일", msg: "혈압이 안정될 수 있습니다." }, { ms: 90 * DAY, label: "90일", msg: "간 지방이 감소합니다." }, { ms: 120 * DAY, label: "120일", msg: "몸 상태가 전반적으로 좋아집니다." }, { ms: 180 * DAY, label: "180일", msg: "피로가 줄어듭니다." }, { ms: 270 * DAY, label: "270일", msg: "정신적 안정감이 증가합니다." }, { ms: 365 * DAY, label: "365일", msg: "심혈관 위험이 감소합니다." }, { ms: 730 * DAY, label: "730일", msg: "건강한 생활이 자리잡습니다." } ];
...
const Data = {
    init() {
        ...
        if (!localStorage.getItem("smokeLogs")) localStorage.setItem("smokeLogs", JSON.stringify([]));
        if (!localStorage.getItem("drinkLogs")) localStorage.setItem("drinkLogs", JSON.stringify([]));
        ...
        if (!localStorage.getItem("smokeRecords")) localStorage.setItem("smokeRecords", JSON.stringify([0, 0, 0]));
        if (!localStorage.getItem("drinkRecords")) localStorage.setItem("drinkRecords", JSON.stringify([0, 0, 0]));
        if (!localStorage.getItem("totalSmokeMoney")) localStorage.setItem("totalSmokeMoney", "0");
        if (!localStorage.getItem("totalDrinkMoney")) localStorage.setItem("totalDrinkMoney", "0");
    },
    ...
    saveSettings(settingsObj) { Object.keys(settingsObj).forEach(key => localStorage.setItem(key, settingsObj[key])); },
    resetData(type) {
    ...
    addLog(type) {
        ...
        let cooldownMs = type === 'smoke' ? (1 * HOUR) : (12 * HOUR);
        let isPenaltyCooldown = elapsedMs < cooldownMs;
        ...
        if (this.getMode(type) === 'quit') {
            ...
            if (type === 'smoke') {
                ...
                let penalty = isPenaltyCooldown ? 0 : Math.floor(this.getSetting("smokePrice", 4500) / 20);
                ...
                let penalty = isPenaltyCooldown ? 0 : this.getSetting("drinkCost", 50000);
    ...
    getReduceStatus(type) {
        ...
        if (type === 'smoke') {
            ...
            startOfWeek.setDate(now.getDate() - dayOfWeek); startOfWeek.setHours(0, 0, 0, 0);
