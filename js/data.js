...
const Data = {
    init() {
        ...
        // [중요] 기존량(Original)과 목표량(Target) 분리
        if (!localStorage.getItem("smokePerDay")) localStorage.setItem("smokePerDay", "10"); // 원래 피우던 양 (금연 모드용)
        if (!localStorage.getItem("smokeTarget")) localStorage.setItem("smokeTarget", "5");  // 줄이기 목표량
        if (!localStorage.getItem("drinkPerWeek")) localStorage.setItem("drinkPerWeek", "2"); // 원래 마시던 횟수 (금주 모드용)
        if (!localStorage.getItem("drinkTarget")) localStorage.setItem("drinkTarget", "1");   // 줄이기 목표 횟수
    ...
    addLog(type) {
        let now = Date.now();
        let logs = JSON.parse(localStorage.getItem(type + "Logs")) || [];
        logs.push(now);
        localStorage.setItem(type + "Logs", JSON.stringify(logs));

        let startTime = parseInt(localStorage.getItem(type + "Start"));
        let elapsedMs = now - startTime;

        let recordsArr = JSON.parse(localStorage.getItem(type + "Records")) || [0, 0, 0];
        recordsArr.push(elapsedMs);
        recordsArr.sort((a, b) => b - a);
        localStorage.setItem(type + "Records", JSON.stringify(recordsArr.slice(0, 3)));

        // 완전 금지 모드일 때만 페널티와 수익 계산 (원래 피우던/마시던 양 기준)
        if (this.getMode(type) === 'quit') {
            let elapsedDays = Math.max(0, elapsedMs / (1000 * 60 * 60 * 24));
            if (type === 'smoke') {
                let earned = Math.floor(elapsedDays * this.getSetting("smokePerDay", 10) * (this.getSetting("smokePrice", 4500) / 20));
                let penalty = Math.floor(this.getSetting("smokePrice", 4500) / 20);
                localStorage.setItem("totalSmokeMoney", (parseInt(localStorage.getItem("totalSmokeMoney")) || 0) + earned - penalty);
            } else {
                ...
                localStorage.setItem("totalDrinkMoney", (parseInt(localStorage.getItem("totalDrinkMoney")) || 0) + earned - penalty);
            }
        }
    ...
    getReduceStatus(type) {
        ...
        if (type === 'smoke') {
            ...
            let target = this.getSetting("smokeTarget", 5); // 목표량 기준
            let remaining = target - countToday;
            return { count: countToday, remaining: remaining, isFail: remaining < 0 };
        } else {
            ...
            let target = this.getSetting("drinkTarget", 1); // 목표량 기준
            let remaining = target - countWeek;
            return { count: countWeek, remaining: remaining, isFail: remaining < 0 };
        }
    },

    getTimeParsed(ms) {
        let sec = Math.floor(ms / 1000), min = Math.floor(sec / 60), hour = Math.floor(min / 60), day = Math.floor(hour / 24);
        return { day, hour: hour % 24, min: min % 60, sec: sec % 60 };
    },
